...
def render_statcast_scatter(batter_df: pd.DataFrame, *, key: str = 'statcast_scatter') -> None:
    if batter_df.empty:
        ...
        return
    fig = px.scatter(batter_df, x='Avg EV', y='Hard Hit %', size='PA', color='Grade', hover_name='Batter', title='Batter Quality of Contact')
    fig.update_layout(height=420, margin=dict(l=20, r=20, t=50, b=20))
    st.plotly_chart(fig, use_container_width=True, config=_get_plotly_config(), key=key)


def render_spray_chart(statcast_df: pd.DataFrame, chart_type: str = 'offensive', *, key: str = 'spray') -> None:
...
def render_runs_by_inning(
    runs_for_by_inning: dict[str, float],
    runs_against_by_inning: dict[str, float],
    *,
    key: str = 'runs_by_inning',
) -> None:
    """Bar chart of runs scored and allowed per inning for the season."""
    if not runs_for_by_inning and not runs_against_by_inning:
        st.info('No runs-by-inning data available.')
        return

    innings = sorted(set(list(runs_for_by_inning.keys()) + list(runs_against_by_inning.keys())),
                     key=lambda x: int(x) if x.isdigit() else 99)

    rf = [runs_for_by_inning.get(i, 0) for i in innings]
    ra = [runs_against_by_inning.get(i, 0) for i in innings]

    fig = go.Figure()
    fig.add_trace(go.Bar(name='Runs Scored', x=innings, y=rf, marker_color='#2196F3'))
    fig.add_trace(go.Bar(name='Runs Allowed', x=innings, y=ra, marker_color='#f44336'))
